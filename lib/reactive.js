	...
	let reactives = {
		...
		*/
		$set(data, options) {
			...
			if (settings.timestamp) reactives.$meta.timestamp = settings.timestamp;
			if (settings.version) reactives.$meta.version = settings.version;

			if (settings.markUpdating) reactives.$meta.isUpdating = false;
		},
		...
		*/
		async $touchLocal() {
			...
			reactives.$touchLocal.promise = settings.supabase
				...
				.then(()=> true); // FIX: Need a promisable here otherwise Supabase can sometimes not execute the query
		...
		*/
		async $touchRemote(data) {
			...
			settings.debug('REMOTE CHANGE', data.new[settings.dataColumn]);
			if (settings.versionColumn && data.new[settings.versionColumn] < reactives.$meta.version) return settings.debug(`Reject server update - version is too low. Server@${data.new[settings.versionColumn]}, Local@${reactives.$meta.version}`);
			if (data.new[settings.timestampColumn] < reactives.$meta.timestamp) return settings.debug(`Reject server update - remote timestamp in the past. Server@${data.new[settings.timestampColumn]}, Local@${reactives.$meta.timestamp}`);

			reactives.$set(data.new[settings.dataColumn], {
				removeKeys: true,
				timestamp: data.new[settings.timestampColumn],
				...(settings.versionColumn && data.new[settings.versionColumn] && {
					version: data.new[settings.versionColumn],
				}),
			});
		},
