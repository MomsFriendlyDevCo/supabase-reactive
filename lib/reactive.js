		...
		async $touchRemote(data) {
			...
			settings.debug(
				...
				settings.versionColumn
					...
					: `Server@${dataTimestamp.toISOString()}, Local@${reactives.$meta.timestamp ? reactives.$meta.timestamp.toISOString() : '[NONE]'}`,
				data.new[settings.dataColumn],
			);

			if (settings.versionColumn && dataVersion <= reactives.$meta.version) return settings.debug('Reject server update - version is too low');
			if (reactives.$meta.timestamp && dataTimestamp <= reactives.$meta.timestamp) return settings.debug('Reject server update - remote timestamp too recent');
