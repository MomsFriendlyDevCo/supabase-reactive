		...
		async $touchRemote(data) {
			...
			settings.debug(
				...
				settings.versionColumn
					...
					: `Server@${dataTimestamp.toISOString()}, Local@${reactives.$meta.timestamp.toISOString()}`,
			...
			if (dataTimestamp <= reactives.$meta.timestamp) return settings.debug('Reject server update - remote timestamp too recent');
